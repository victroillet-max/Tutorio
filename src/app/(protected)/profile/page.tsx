...
import { Mail, Calendar, Award, BookOpen, CheckCircle, Clock, Zap, Crown, ArrowUpRight, CreditCard } from "lucide-react";
...
export default async function ProfilePage() {
  ...
  const activeSubscriptions = (subscriptions as UserCourseSubscription[] | null)?.filter(
        ...
        {activeSubscriptions.length > 0 ? (
          <div className="space-y-3">
            {activeSubscriptions.map((sub) => {
              const isAdvanced = sub.tier_slug === 'advanced';
              ...
              return (
                <div 
                  ...
                  className="flex items-center gap-4 p-4 rounded-xl bg-[var(--background-secondary)]"
                >
                  <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${
                    isAdvanced ? 'bg-[var(--accent)]/10 text-[var(--accent)]' : 'bg-blue-100 text-blue-600'
                  }`}>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-sm text-[var(--foreground-muted)]">
                      {sub.tier_name} - Renews {periodEnd.toLocaleDateString()}
                    </p>
                  </div>
                  <div className="flex items-center gap-2">
                    {!isAdvanced && (
                      <Link
                        href={`/pricing?course=${sub.course_slug}`}
                        className="flex items-center gap-1 px-3 py-1.5 text-xs font-semibold text-[var(--accent)] border border-[var(--accent)] rounded-lg hover:bg-[var(--accent)] hover:text-white transition-colors"
                      >
          ...
          <div className="text-center py-8">
            ...
            <Link
              href="/courses"
              className="inline-flex items-center gap-2 px-4 py-2 bg-[var(--accent)] text-white text-sm font-semibold rounded-lg hover:bg-[var(--accent-dark)] transition-colors"
            >
              Browse Courses
